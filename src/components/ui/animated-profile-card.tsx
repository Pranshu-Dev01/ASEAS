...
// Simple fallback for theme detection if next-themes is missing
const useTheme = () => {
  return { resolvedTheme: "dark" }; // Default to dark since our site is dark
};

// Simplified useGSAP fallback to remove external dependency
function useGSAP(callback: () => void, { scope }: { scope?: React.RefObject<HTMLDivElement | null> }) {
  useEffect(() => {
  ...
  (
    ...
    return (
      <div
        ...
        className={cn(
          "relative w-full overflow-hidden rounded-3xl border-2 h-full pointer-events-auto",
          className
        )}
        {...rest}
      >
