    ...
    return (
        <ul className="grid grid-cols-1 grid-rows-none gap-4 md:grid-cols-12 md:grid-rows-2 lg:gap-4 xl:max-h-[34rem] xl:grid-rows-2">
            <GridItem
                ...
                title="Where handwriting meets AI"
                description="Transforming physical student work into digital intelligence with seamless integration."
            />
            <GridItem
                area="md:[grid-area:1/7/2/13] xl:[grid-area:1/5/2/13]"
                icon={<Settings className="h-4 w-4" />}
                title="Absolute Precision"
                description="Semantic understanding with absolute precision, ensuring fair and accurate grading every time."
            />
            <GridItem
                area="md:[grid-area:2/1/3/7] xl:[grid-area:2/1/3/8]"
                icon={<Lock className="h-4 w-4" />}
                title="Unlocking Potential"
                description="Unlocking every student’s true potential by removing evaluation bias and teacher fatigue."
            />
            <GridItem
                area="md:[grid-area:2/7/3/13] xl:[grid-area:2/8/3/13]"
                icon={<Sparkles className="h-4 w-4" />}
                title="Smart Education"
                description="Actionable insights for smarter education, turning raw evaluation data into pedagogical growth."
            />
        </ul>
    );
}
